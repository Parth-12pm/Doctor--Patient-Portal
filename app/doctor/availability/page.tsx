...
export default function DoctorAvailabilityPage() {
      ...
      if (response.ok) {
        ...
        if (data.doctor) {
          setAvailability({
            ...
            blockedDates:
              data.doctor.blockedDates?.map((date: string) => new Date(date).toISOString().split("T")[0]) || [],
          })
        }
      }
    } catch (error) {
  ...
  const handleTimeSlotToggle = (day: string, timeSlot: string) => {
    setAvailability((prev) => {
      ...
      const dayIndex = newAvailability.availableSlots.findIndex((slot) => slot.day === day)
      ...
      if (dayIndex === -1) {
        ...
        if (currentSlots.includes(timeSlot)) {
          ...
          newAvailability.availableSlots[dayIndex].timeSlots = currentSlots.filter((slot) => slot !== timeSlot)
    ...
    try {
      const response = await fetch("/api/doctors/availability", {
        ...
        body: JSON.stringify({
          ...
          blockedDates: availability.blockedDates.map((date) => new Date(date).toISOString()),
        }),
      })
  ...
  const getDaySlots = (day: string) => {
    const daySlot = availability.availableSlots.find((slot) => slot.day === day)
      ...
      <div className="container mx-auto p-6 max-w-6xl">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">Set your working hours and blocked dates</p>
          </div>
        </div>
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Set your available time slots for each day (9:00 AM - 7:00 PM)</CardDescription>
              </CardHeader>
                    ...
                    <div key={key} className="space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <Switch checked={isEnabled} onCheckedChange={(checked) => handleDayToggle(key, checked)} />
                      ...
                      {isEnabled && (
                        <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2 ml-8">
                          {TIME_SLOTS.map((timeSlot) => (
                            <Button
                              key={timeSlot}
                              variant={daySlots.includes(timeSlot) ? "default" : "outline"}
                              size="sm"
                              onClick={() => handleTimeSlotToggle(key, timeSlot)}
                              className="text-xs"
                            >
                              {timeSlot}
                            </Button>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {availability.availableSlots.reduce((total, day) => total + day.timeSlots.length, 0)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Max Daily Appointments</span>
                  <span className="font-medium">15</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Blocked Dates</span>
                  <span className="font-medium">{availability.blockedDates.length}</span>
                </div>
              </CardContent>
            </Card>

            {/* Save Button */}
            <Button onClick={saveAvailability} disabled={isSaving} className="w-full">
              {isSaving ? (
                <>
